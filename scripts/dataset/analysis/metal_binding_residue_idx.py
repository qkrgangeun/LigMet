...
# 결과 CSV 파일 경로
output_csv = "/home/qkrgangeun/LigMet/benchmark/mionic/metal_binding_sites_residx_NOSSE_3.0.csv"
...
def process_pdb(pdb_file, cutoff=2.8):
    """
    ...
    for metal_idx, metal_pos in enumerate(metal_positions):
        ...
        # 필터링된 원자 인덱스를 이용해 잔기 이름과 잔기 인덱스를 그대로 가져옴
        # 잔기 이름과 인덱스를 'residue_name:residue_idx' 형식으로 저장
        binding_residues = [f"{atom_residues[i]}:{residue_idxs[i]}" for i in n_o_s_neigh_idx]
        ...
        # Cluster ID 정보는 제거되었음
        # 예시: metal_position은 소수점 3자리까지 반올림하여 기록
        results.append([
            pdb_id,
            metal_type,
            [round(coord, 3) for coord in metal_pos.tolist()],
            binding_residues,  # residue_name:residue_idx 형식으로 저장
            unique_chains,     # 체인 정보
        ])
    return results
...
if __name__ == "__main__":
    ...
    with open("/home/qkrgangeun/LigMet/benchmark/test_pdb.txt", "r") as f:
    ...
    with open(output_csv, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["PDB ID", "Metal Type", "Metal Position", "Binding Residues", "Binding Chains"])
